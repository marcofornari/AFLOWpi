...
def _grab__code(oneCalc,ID):
	outString = ''
	codeRegex=ur'Program PWSCF ([A-Za-z0-9.]*)\s*'
...
def _grab__Bravais_lattice_orig(oneCalc,ID):
	outString = ''
	ibravRegex = ur'bravais-lattice index\s*=\s*([-0-9]*)'
...
def _grab__energy_cutoff(oneCalc,ID):
	outString = ''
	energyCutoffRegex = ur'kinetic-energy cutoff\s*=\s*([-0-9.]*)'
...
def _grab__natoms(oneCalc,ID):
	outString = ''
	natomsRegex = ur'number of atoms[/]*cell\s*=\s*([0-9]*)'
...
def _grab__nspecies(oneCalc,ID):
	outString = ''
	nspeciesRegex = ur'number of atomic types\s*=\s*([0-9]*)'
...
def _grab__pressure(oneCalc,ID):
	outString = ''
	pressureRegex = ur'\(kbar\)\s*P=\s*([-0-9.]*)'
...
def _grab__composition(oneCalc,ID):
	...
	return OrderedDict(sorted(atom_list.items(), key=lambda (key, value): int(key.split('_')[1]))).keys()
...
def _grab__volume_cell(oneCalc,ID):
	outString = ''
	volRegex = ur'unit-cell volume\s*=\s*([0-9.]*)'
...
def _write_properties_for_one(all_properties,dest):
	...
	for prop_list_name,prop_list in out_props.iteritems():
		property_dict={}
		for i in prop_list:
			if i in all_properties.keys():
...
#
	...
	if discard_incomplete==True:
		...
		for ID,oneCalc in calcs_copy.iteritems():
			wf_len = len(oneCalc['_AFLOWPI_WORKFLOW_'])
			if wf_len<longest_workflow:
				del calcs[ID]
...
def load_last(project,set_name='',config=''):
	...
	for i in range(len(calc_list)):
		for k,v in calc_list[i].iteritems():
