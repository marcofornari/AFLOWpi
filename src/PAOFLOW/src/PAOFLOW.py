...
#                kq[:,n]=np.dot(kq[:,n],b_vectors)
            ...
            if band_topology:
                ...
                eff_mass=True
                do_topology_calc(HRs,SRs,non_ortho,kq,E_kp,v_kp,R,Rfft,R_wght,idx,alat,b_vectors,nelec,bnd,Berry,ipol,jpol,spin_Hall,spol,do_spin_orbit,sh,nl,eff_mass,inputpath,npool,a_vectors)
...
#                        reset=time.time()
    ...
    E_k_temp = gather_full(np.ascontiguousarray(E_k[:,:bnd]),npool)
    if rank==0:
        E_k_temp=np.sort(np.ravel(E_k_temp))
    ...
    if Boltzmann:
        ...
        E_k_range = np.ascontiguousarray(E_k[E_k_mask[0],E_k_mask[1],E_k_mask[2]])-efc
...
#                        S[:,:,n] = -1.0*np.dot(inv_L0[:,:,n],L1[:,:,n])
                    for i in range(3):
                        for j in range(3):
                            for a in range(3):
                                S[i,j,n] += -1.0*inv_L0[a,i,n]*L1[a,j,n]
...
#                        kappa[:,:,n] = (L2[:,:,n] - temp*11604.52500617*np.dot(L1[:,:,n],np.dot(inv_L0[:,:,n],L1[:,:,n])))*1.e6
                        kappa[:,:,n] = L2[:,:,n]*1.e6
                        for i in range(3):
                            for j in range(3):
                                for a in range(3):
                                    for b in range(3):
                                        kappa[i,j,n] += (-temp*11604.52500617*L1[i,a,n]*inv_L0[b,a,n]*L1[b,j,n])*1.e6
                    except:
                        kappa[:,:,n] = 0.0
