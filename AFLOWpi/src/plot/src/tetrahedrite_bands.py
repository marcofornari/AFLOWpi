...
#       pylab.yticks(numpy.arange(yLim[0],yLim[1]+1,2))
     ...
     #Print path labels to band structure x-axis
        try:
                ...
                pylab.xticks(bars,SymPrint, fontsize = 36)
        except Exception as e:
...
#                pylab.xlabel('arbitrary units',fontsize = 20)
                ...
                atom_count = AFLOWpi.retr._getAtomNum(oneCalc['_AFLOWPI_INPUT_'],strip=False)
                for fi in range(len(pDOSNoPath)):
                        ...
                        try:
                                for jjjjj in range(len(floatdos)):
                                        floatdos[jjjjj]/=atom_count[species]
...
#                       ax2.legend(handles[::-1], labels[::-1],fontsize=14,loc=1)
                        ax2.legend(handles, labels,fontsize=24,loc=1)
...
#       t = pylab.gcf().text(0.5,0.92, figtitle,fontsize=20,horizontalalignment='center') #[x,y]
        ...
        if SBC:
                ...
                for item in range(len(labs)):
                        if item==len(labs)-1:
                                labs[item] =  '    $0$'                 
                        else:
                                labs[item] =  '$-10^{%s}$'%(len(labs)-int(item)-1)
